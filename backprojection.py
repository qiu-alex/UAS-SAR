...
def interpolate_position(proportion, pos_start,
                         pos_end):  # not really necessary but interpolates x, y, and z positions; maybe use np.interp()?
...
def takeClosest(motionCaptureTime, value, motionCapture):  # returns the x, y, z radar position at a certain time
    ...
    return motionCapture[
        -1]  # this accounts for the extra radar pulses that were when the radar wasn't moving, so the motion capture x,y,z would remiain the same


def mc_start(motionCapture):  # input velocity instead to say that it's moving?
    for i in range((len(motionCapture) - 101)):
        if abs(motionCapture[i + 100][0] - motionCapture[i][0]) > 0.03 or abs(
                motionCapture[i + 100][1] - motionCapture[i][1]) > 0.03 or abs(
                motionCapture[i + 100][2] - motionCapture[i][2]) > 0.03:
            return i


def mc_end(motionCapture):  # are all the intervals the same??
    for j in range((len(motionCapture) - 101)):
        if abs(motionCapture[len(motionCapture) - j - 101][0] - motionCapture[len(motionCapture) - j - 1][
            0]) > 0.03 or abs(
                motionCapture[len(motionCapture) - j - 101][1] - motionCapture[len(motionCapture) - j - 1][
                    1]) > 0.03 or abs(
                motionCapture[len(motionCapture) - j - 101][2] - motionCapture[len(motionCapture) - j - 1][2]) > 0.03:
            return len(motionCapture) - j


def pulse_start(pulses):  # TEST LOGARITHMIC SCALE AS WELL and increase degrees of freedom for when drone is moving, fix to get the end of 1.0s
    pulses = pulses + 2 ** (np.floor(np.log2(np.max(np.abs(pulses)))) + 1)  # 2 ** 17
...
def get_platform_position(motionCapture, motion_capture_time, pulse_time_stamp):
    ...
    for i in range(len(pulse_time_stamp)):
        (platform_position[i][0], platform_position[i][1], platform_position[i][2]) = takeClosest(motion_capture_time,
                                                                                                  pulse_time_stamp[i],
                                                                                                  motionCapture)
...
def main(args):
    ...
    platform_pos, pulse_time_stamp = get_platform_position(motionCapture, motion_capture_time,
                                                           pulse_time_stamp)  # rearranges x, y, and z correspondingly
